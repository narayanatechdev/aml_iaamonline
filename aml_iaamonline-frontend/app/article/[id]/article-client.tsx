...
export default function ArticleClient() {
  ...
  useEffect(() => {
    const signedIn = isUserAuthenticated() || isAdminAuthenticated();
    ...
    if (signedIn && id) {
      (async () => {
        try {
          ...
          const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/articles/${id}/access`, {
            headers: { Accept: 'application/json', Authorization: `Bearer ${token}` },
          });
          if (!res.ok) return; // fail open on API errors
          const json = await res.json();
          setAccessCheck(json.data);
        } catch {
          // fail open
        }
      })();
    ...
    const fetchArticle = async () => {
      try {
        ...
        const overlay: Partial<FeaturedArticle> = {
          ...
          ...(data.pdf_downloads != null ? { pdf_downloads: data.pdf_downloads } : {}),
        } as Partial<FeaturedArticle> & { pdf_downloads?: number };
  ...
  useEffect(() => {
    if (!id) return;
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/articles/${id}/view`, { method: 'POST' }).catch(() => {});
  }, [id]);

  const recordDownload = () => {
    if (!id) return;
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/articles/${id}/download`, { method: 'POST' }).catch(() => {});
  };
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              {hasFullAccess ? (
                <a
                  ...
                  {...(article.pdf_url ? { onClick: recordDownload } : { onClick: (e) => e.preventDefault() })}
                >
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm text-[#3a4a6a]">
                    You have reached your article allowance for{' '}
                    {(accessCheck?.remaining_month ?? 1) > 0 ? 'today' : 'this month'}. Articles you
                    already opened today remain available.
                  </p>
                </div>
              ) : (
                <div className="text-center">
        ...
        <div className="lg:col-span-3 order-1 lg:order-2">
          ...
          <div id="header" className="py-6 mb-8 border-b border-gray-200 scroll-mt-36">
            ...
            <RichText
              as="h1"
              html={article.title}
              className="text-3xl text-[#0f1a2e] mb-4 leading-snug font-bold"
            />
                          ...
                          <span key={author.id} className="inline-flex items-baseline gap-1">
                            ...
                            {author.orcid && (
                              <a
                                ...
                                aria-label={`ORCID profile of ${author.name}`}
                                className="self-center inline-flex items-center justify-center w-4 h-4 rounded-full bg-[#A6CE39] text-white text-[9px] font-bold leading-none hover:opacity-80"
                              >
                                iD
                              </a>
                            )}
                            {author.email && (
                              <a
                                ...
                                className="self-center text-[#0f2d6b] hover:text-[#c9a227]"
                              >
                                ✉
                              </a>
                            )}
            ...
            {/* Metadata */}
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 py-4 mb-5 border-y border-gray-200">
              {[
                ...
                { label: "PDF Downloads", value: pdfDownloads.toLocaleString() },
              ].map((m) => (
                <div key={m.label}>
            ...
            <div className="flex flex-col md:flex-row gap-8 items-start">
              ...
              <div className="flex-grow">
                {showAbstract ? (
                  ...
                  <div className="bg-[#f0f4fb] border border-[#0f2d6b]/10 rounded-lg p-5 text-center">
                    <Lock className="w-5 h-5 text-[#c9a227] mx-auto mb-2" />
                    <p className="text-sm text-[#3a4a6a]">
          ...
          <div id="keywords" className="py-6 mb-8 scroll-mt-36">
            ...
            <div className="flex flex-wrap gap-2">
              {article.keywords.map((kw) => (
                <Link
                  key={kw}
                  href={`/search?keyword=${encodeURIComponent(richTextToPlain(kw))}`}
                  ...
                  return (
                    <Link key={rel.id} href={`/article/${rel.id}`} className="block bg-white rounded-lg p-4 border border-gray-200 hover:shadow-lg transition-all">
                      <RichText
                        as="p"
                        html={rel.title}
                        className="text-black text-base leading-snug mb-2 hover:text-[#0f2d6b] font-semibold"
                      />
